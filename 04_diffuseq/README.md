## Генерация через latent replacement

В данном подходе условие задается не через архитектуру, а через сам процесс генерации: латенты, соответствующие промпту, фиксируются на каждом шаге обратного процесса, а зашумление и расшумление выполняются только на латентах продолжения.  Подход адаптирован из метода DiffuSeq.
...
- `conda create --name actdm_diffuseq python=3.9`
- `conda activate actdm_diffuseq`
- `conda install pip`
...
После подсчета статистик и обучения декодера запустите обучение диффузионной модели с latent replacement:
```
