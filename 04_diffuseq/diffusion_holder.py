...
class DiffusionRunner:
    ...
    def train_step(self, batch):
        ...
        loss_dict, stat_dict = self.calc_loss(x0_src=src_x, x0_trg=trg_x, batch=batch)
    ...
    def validate(self) -> None:
        ...
        for batch in self.valid_loader:
            ...
            loss_dict, _ = self.calc_loss(x0_src=src_x, x0_trg=trg_x, batch=batch)
            for k, v in loss_dict.items():
                if k in valid_loss:
    ...
    def calc_loss(self, x0_src, x0_trg, batch=None, eps=1e-5):
        batch_size = x0_trg.size(0)
        t = self.sample_time(batch_size, eps=eps)

        marg_forward = self.dynamic.marginal(x0_trg, t)
        y_t, noise = marg_forward['x_t'], marg_forward['noise']

        if self.config.is_conditional and x0_src is not None:
            src_len = x0_src.shape[1]
            z_t = torch.cat([x0_src, y_t], dim=1)
            attn_src = batch.get("attention_mask_src")
            attn_trg = batch.get("attention_mask_trg")
            combined_mask = torch.cat([attn_src, attn_trg], dim=1) if attn_src is not None else None
        else:
            src_len = 0
            z_t = y_t
            combined_mask = None
        ...
        if self.config.use_self_cond and random.random() > 0.5:
            with torch.autocast(device_type='cuda', dtype=torch.bfloat16):
                with torch.no_grad():
                    z0_sc = self.ddp_score_estimator(
                        x_t=z_t, time_t=t, cond=None,
                        attention_mask=combined_mask, cond_mask=None,
                        x_0_self_cond=x_0_self_cond
                    ).detach()
                    if src_len > 0:
                        z0_sc[:, :src_len, :] = x0_src
                    x_0_self_cond = z0_sc

        with torch.autocast(device_type='cuda', dtype=torch.bfloat16):
            z0_pred = self.ddp_score_estimator( 
                x_t=z_t, time_t=t, cond=None,
                attention_mask=combined_mask, cond_mask=None,
                x_0_self_cond=x_0_self_cond
            )

        x0_trg_pred = z0_pred[:, src_len:, :]
        loss_x_0 = mse_loss(x0_trg, x0_trg_pred, None)
        loss_dict = {'total_loss': loss_x_0, 'loss_x_0': loss_x_0}

        with torch.no_grad():
            stat_dict = {}
            mask = None
            for key, val in get_stat(x0_trg, mask).items():
                stat_dict[f"clean_x_{key}"] = val
            for key, val in get_stat(x0_trg_pred.detach(), mask).items():
                stat_dict[f"x_0_{key}"] = val
            mask_trg = batch["attention_mask_trg"]
            for key, val in get_stat(x0_trg, mask_trg).items():
                stat_dict[f"clean_x_woSPT_{key}"] = val
            for key, val in get_stat(x0_trg_pred, mask_trg).items():
                stat_dict[f"x_0_woSPT_{key}"] = val
            ...
            with torch.autocast(device_type='cuda', dtype=torch.bfloat16), torch.no_grad():
                src_enc = self.encoder(
                    **{
                        "input_ids": batch["input_ids_src"],
                        "attention_mask": batch["attention_mask_src"]}
                ) if self.config.is_conditional else None

            gen_text = self.generate_text_batch(
                batch_size=len(batch["text_trg"]),
                src_enc=src_enc,
                src_mask=batch.get("attention_mask_src"),
            )[0]
    ...
    @torch.no_grad()
    def generate_text_batch(self, batch_size, src_enc=None, src_mask=None):
        pred_embeddings = self.pred_embeddings(batch_size, src_enc=src_enc, src_mask=src_mask)
    ...
    def pred_logits(self, pred_embeddings):
        ...
        output = self.decoder(pred_embeddings, cond_x=None, cond_mask=None)
        return output

    @torch.no_grad()
    def pred_embeddings(self, batch_size, src_enc=None, src_mask=None):
        self.score_estimator.eval()
        hidden_size = self.encoder.encoder.config.hidden_size
        trg_len = self.config.data.max_sequence_len
        y_T = self.dynamic.prior_sampling((batch_size, trg_len, hidden_size)).to(self.device)

        if self.config.is_conditional and src_enc is not None:
            src_len = src_enc.shape[1]
            z = torch.cat([src_enc, y_T], dim=1)
            if src_mask is not None:
                trg_ones = torch.ones(batch_size, trg_len, device=src_mask.device, dtype=src_mask.dtype)
                combined_mask = torch.cat([src_mask, trg_ones], dim=1)
            else:
                combined_mask = None
        else:
            src_len = 0
        ...
        x_0_self_cond = torch.zeros_like(z)
        eps_t = 0.01
        timesteps = torch.linspace(self.dynamic.T, eps_t, self.dynamic.N + 1, device=self.device)

        for idx in tqdm(range(self.dynamic.N)):
            t = timesteps[idx] * torch.ones(batch_size, device=self.device)
            next_t = timesteps[idx + 1] * torch.ones(batch_size, device=self.device)

            output = self.diff_eq_solver.step(
                x_t=z, t=t, next_t=next_t,
                cond=None, cond_mask=None,
            ...
            if src_len > 0:
                z[:, :src_len, :] = src_enc
                z_mean[:, :src_len, :] = src_enc
                x_0_self_cond[:, :src_len, :] = src_enc

        return z_mean[:, src_len:, :] 
