...
def create_config(args):
    ...
    config.architecture_type = getattr(args, "architecture_type", ARCHITECTURE_TYPES[0])
    if config.architecture_type not in ARCHITECTURE_TYPES:
        raise Exception(
            f"Unknown architecture_type: {config.architecture_type}. "
