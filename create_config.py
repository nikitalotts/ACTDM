...
TRAINING_RECIPE = {
    "gpt": {
        ...
        # сопоставимости. За один шаг 512 в память не влезает, поэтому
        # 32 x 4 карты x 4 шага накопления.
        #
        # Почему 32, а не 64: пробник давал предел 82 на карту и 81% памяти при
        # 64, но это БЕЗ буферов DDP и без генерации на eval. Боевой smoke на
        # четырех картах с батчем 64 дошел до 90% памяти и упал по OOM сразу
        # после первого eval (задание 4265702). При 32 расход вдвое меньше,
        # запас есть; цена -- около 5% скорости.
        "effective_batch": 512,
