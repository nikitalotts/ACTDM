...
| `TIME_SCALE` | `1.0` (по умолчанию), `1000.0` | Масштаб `t` в эмбеддинге времени классификатора, только для `ARCH_TYPE=guidance` |
...
**`TIME_SCALE`.** Классификатор получает номер шага через синусоидальный эмбеддинг, но `t` здесь непрерывный и лежит в `[0.001, 1]`, а не является номером шага `0..T`. При `TIME_SCALE=1.0` аргументы синусов не превышают 1, и эмбеддинги крайних `t` почти совпадают — косинусная близость `t=0.001` и `t=1.0` равна **0.973**, то есть уровень шума практически не доходит до классификатора. `TIME_SCALE=1000.0` возвращает разрешение как в DDPM с `T=1000`: та же близость падает до **0.17**.

По умолчанию множитель выключен — поведение в точности такое, каким было раньше. Включать имеет смысл при обучении классификатора с нуля:
```
TIME_SCALE=1000.0 bash train_conditional_encoder_combined.sh
TIME_SCALE=1000.0 ARCH_TYPE=guidance CG_SCALE=10.0 bash eval_diffusion.sh
```
Варианты лежат в разных файлах (`-ts1000` в имени), а при попытке применить классификатор с одним `TIME_SCALE` в конфиге с другим запуск падает с явным сообщением: форма весов при этом не меняется, и без проверки расхождение прошло бы молча.
...
35 проверок, сверяющих реализацию с первоисточниками: partial noising и anchoring из DiffuSeq, подача условия через cross-attention из GENIE, формулы classifier guidance и схемы обучения классификатора из ВКР (раздел 4.1), маскирование промпта в лоссе у авторегрессионного baseline. Часть тестов прогоняет реальные тензоры через `calc_loss` / `pred_embeddings`; им нужен GPU, без него они помечаются skip.
