...
# Отладочная печать на КАЖДЫЙ батч (около 20 строк) при 35815 шагах прогона
# давала бы под миллион строк в slurm-логе. Хуже того, печать тензоров и .item()
...
def loss_step(epoch, batch, tokenizer, encoder, cond_encoder, config, device, eval=False, batch_idx=0, curriculum_progress=None):
...
def main():
    ...
    args.augmentation_scheme = "shuffled"
