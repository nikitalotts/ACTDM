...
class DiffusionRunner:
    ...
    def __save_checkpoint(self, save_path):
        ...
        if self.cond_encoder is not None:
            checkpoint["cond_encoder"] = self.cond_encoder.state_dict()
    ...
    def train(self) -> None:
        ...
        print(f'#INFO finished set valid data generator, time: {time.time() - start_time:.4f}s')

        self.train_range = trange(self.step + 1, self.config.training.training_iters + 1)
        self.train_range_iter = iter(self.train_range)

        while True:
            print('#INFO enter while true')

            start_time = time.time()
            self.set_train_data_generator()
            print(f'#INFO finished set train data generator, time: {time.time() - start_time:.4f}s')
            ...
            print(f'#INFO finished set model to train mode, time: {time.time() - start_time:.4f}s')

            start_time = time.time()
            self.train_epoch()
            print(f'#INFO finished train epoch, time: {time.time() - start_time:.4f}s')

            print(f'#INFO CURRENT STATE: step={self.step}, target_iters={self.config.training.training_iters}')
        ...
        start_time = time.time()
        self.score_estimator.eval()
        print(f'#INFO set model to eval mode, time: {time.time() - start_time:.4f}s')

        start_time = time.time()
        self.save_checkpoint(last=True)
        print(f'#INFO saved checkpoint, time: {time.time() - start_time:.4f}s')

        start_time = time.time()
        self.switch_to_ema()
        print(f'#INFO switched to EMA, time: {time.time() - start_time:.4f}s')
    ...
    def train_epoch(self):
        print('#INFO in train_epoch', self.step, self.config.training.training_iters)
        print('#INFO train_loader len', len(self.train_loader))
        for _, batch in enumerate(self.train_loader):
            if self.step >= self.config.training.training_iters:
                return
        ...
        print('#INFO finished TRIAN EPOCH\n')
    ...
    def train_step(self, batch):
        ...
        if self.step % 10 == 0:
            ...
            for k, v in stat_dict.items():
                self.log_metric("statistics-old", k, v.item())

        return loss_dict, stat_dict

    @torch.no_grad()
    def validate(self) -> None:
        print('#INFO ENTER VALIDAtiON\n')
        self.set_valid_data_generator()
            ...
            with torch.autocast(device_type='cuda', dtype=torch.bfloat16):
                ...
                with torch.no_grad():
                    x_0_self_cond = self.ddp_score_estimator(
                        ...
                        attention_mask=mask, 
                        cond_mask=batch.get("attention_mask_src") if cond_x is not None else None,
                        x_0_self_cond=x_0_self_cond
                    ).detach()
    ...
    def generate_text_conditional(self, dataset_name: str, split: str):
        ...
        for batch in loader:
            ...
            if self.config.is_conditional or self.guidance_scale > 0:
                ...
                cond_mask = batch["attention_mask_src"]
            else:
                src_x = None
                cond_mask = None
    ...
    def estimate(self, split: str):
        ...
        for dataset_name in self.config.data.datasets.datasets_list:
            print('#INFO generating result_dict for:', dataset_name)
            if dist.is_initialized():
        ...
        if not dist.is_initialized() or dist.get_rank() == 0:
            ...
            file_name = f"{self.step}-N={self.config.dynamic.N}-len={len(result_list)}-guidance_scale={str(self.guidance_scale)}-seed={self.config.seed}-{str(random.randint(100, 999))}.json"
        ...
        metrics_dict = dict()
        for dataset_name in self.config.data.datasets.datasets_list:
            print('#INFO metrics for:', dataset_name)
        ...
        print('#INFO FINISHED MAIN ESIMATE')
        self.switch_back_from_ema()
        print('#INFO FINISHED switch_back_from_ema')
        self.ddp_score_estimator.train()
        print('#INFO FINISHED ddp_score_estimator')
        self.score_estimator.train()
        print('#INFO FINISHED score_estimator.train()')
    ...
    def _aggregate_and_save(self, all_runs, split, num_seeds, base_seed):
        ...
        file_name = (f"statistical_eval-{split}-num_seeds={num_seeds}"
                     f"-base_seed={base_seed}-step={self.step}-N={self.config.dynamic.N}"
                     f"-gs={self.guidance_scale}.json")
        save_path = os.path.join(prefix_folder, file_name)
    ...
    @torch.no_grad()
    def generate_texts_extended(self, dataset_name: str, split: str):
        dt = next(get_dataset_iter(self.config, dataset_name, split=split))
        loader = DataLoader(
            dt,
            num_workers=20,
        ...
        for batch in loader:
            ...
            target_texts = batch["text_trg"]
            
            extended_results = self._generate_with_intermediate_states(
                batch_size=len(batch["text_trg"]),
                cond_x=src_x,
                cond_mask=cond_mask,
                attention_mask=None,
            ...
            for i in range(len(batch["text_trg"])):
                ...
                for step_info in extended_results["steps"]:
                    example["steps"].append({
                        "step_idx": step_info["step_idx"],
                        "t": step_info["t"],
                        "text": step_info["texts"][i],
                        "classifier_prob": step_info["classifier_probs"][i] if step_info["classifier_probs"] is not None else None,
                        "classifier_logit": step_info["classifier_logits"][i] if step_info["classifier_logits"] is not None else None
                    })
        ...
        return {
            ...
            "statistics": extended_results["statistics"],
            "final_bert_score_mean": extended_results["final_bert_score_mean"]
        }

    @torch.no_grad()
    def _generate_with_intermediate_states(
            self,
            batch_size,
            cond_x=None,
            cond_mask=None,
        ...
        print("\n" + "="*80)
        print("GENERATION WITH INTERMEDIATE STATES")
        print("="*80)
        
        final_texts = None
        
        for idx in tqdm(range(self.dynamic.N), desc="Generating with intermediate states"):
            t = timesteps[idx]
            next_t = timesteps[idx + 1]
            ...
            x, x_mean = output["x"], output["x_mean"]
            x_0_pred = output["x_0"]
            x_0_self_cond = x_0_pred
            
            texts = self._decode_embeddings_to_text(x_0_pred, cond_x=cond_x, cond_mask=cond_mask)
            
            if idx == self.dynamic.N - 1:
                final_texts = texts
            
            if classifier_probs is not None and classifier_logits is not None:
                probs_tensor = torch.tensor(classifier_probs)
                logits_tensor = torch.tensor(classifier_logits)
                
                step_stats = {
                    "step_idx": idx,
                    "t": t.item(),
                    "prob_mean": probs_tensor.mean().item(),
                    "prob_median": probs_tensor.median().item(),
                    "prob_std": probs_tensor.std().item(),
                    "logit_mean": logits_tensor.mean().item(),
                    "logit_median": logits_tensor.median().item(),
                    "logit_std": logits_tensor.std().item(),
                }
                statistics.append(step_stats)
                
                if idx % 10 == 0 or idx == self.dynamic.N - 1:
                    print(f"\n{'='*80}")
                    print(f"Step {idx}/{self.dynamic.N}, t={t:.4f}")
                    print(f"{'='*80}")
                    print(f"Classifier Probabilities:")
                    print(f"  Mean:   {step_stats['prob_mean']:.4f}")
                    print(f"  Median: {step_stats['prob_median']:.4f}")
                    print(f"  Std:    {step_stats['prob_std']:.4f}")
                    print(f"\nClassifier Logits:")
                    print(f"  Mean:   {step_stats['logit_mean']:.4f}")
                    print(f"  Median: {step_stats['logit_median']:.4f}")
                    print(f"  Std:    {step_stats['logit_std']:.4f}")
                    print(f"{'='*80}")
        ...
        final_bert_scores = None
        final_bert_score_mean = None
        
        if target_texts is not None and final_texts is not None:
            print("\n" + "="*80)
            print("COMPUTING FINAL BERT-SCORE")
            print("="*80)
            
            valid_pairs = [(pred, ref) for pred, ref in zip(final_texts, target_texts) if pred and ref]
            
            if valid_pairs:
                valid_preds, valid_refs = zip(*valid_pairs)
                
                final_bert_score_mean = compute_metric(
                    metric_name="bert-score",
                    predictions=list(valid_preds),
                    references=list(valid_refs)
                )
                
                print(f"\nFinal BERT-Score (mean): {final_bert_score_mean:.4f}")
                print("="*80)
        ...
        return {
            ...
            "final_bert_scores": final_bert_scores,
            "final_bert_score_mean": final_bert_score_mean
        }
    @torch.no_grad()
    def _decode_embeddings_to_text(self, embeddings, cond_x=None, cond_mask=None):
        output = self.pred_logits(embeddings, cond_x=cond_x, cond_mask=cond_mask)
        tokens = output.argmax(dim=-1)
        ...
        texts = self.tokenizer.batch_decode(tokens_list, skip_special_tokens=True)
        return texts


    @torch.no_grad()
    def _compute_classifier_outputs(self, x_t, cond_x, t, cond_mask=None):
        logits = self.cond_encoder(
            src_embeds=cond_x,
            noisy_trg_embeds=x_t,
            t=t,
            src_mask=cond_mask
        ) 
        ...
        return logits.cpu().tolist(), probs.cpu().tolist()
