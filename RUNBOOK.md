...
Принцип: на scratch — всё **восстановимое** (датасеты, кэши), в home — всё,
что жалко потерять (чекпоинты, результаты). Настроить один раз, до прогрева:

```bash
mkdir -p /scratch/$USER/actdm/datasets
ln -s /scratch/$USER/actdm/datasets ~/ACTDM/datasets     # путь к репе поправить
```

- `datasets/` (симлинк на scratch): нарезанная wikipedia + статистики +
  декодеры + классификаторы. Данные восстановимы повторным запуском стадий;
  arrow-шарды на NVMe читаются быстрее.
- Кэш HuggingFace настраивается **автоматически**: если на машине есть
  `/scratch/$USER`, все точки входа (prefetch_offline.py, run_wikipedia.sh и
  все sbatch-задания через run_flags.sh -> hf_env.sh) используют
  `/scratch/$USER/hf_cache`. Каждый скрипт печатает `HF_HOME=...` при старте —
  сверить, что прогрев и задания видят один путь. Переопределение: обычный
  `export HF_HOME=...` (заданный снаружи путь ничем не перетирается).
  Кэш (~13 GB) восстановим повторным `prefetch_offline.py` на login-ноде.
- Репа, `checkpoints/`, `generated_texts/`, `wandb/` — в home: чекпоинты
  диффузии невосстановимы без повторного обучения.
- **После стадий 3.2, 3.3 и 3.5 копировать мелкие артефакты в home** — если
  scratch почистят, не придется переобучать декодеры и классификаторы:

```bash
mkdir -p ~/actdm_backup && cp /scratch/$USER/actdm/datasets/wikipedia/*.pth ~/actdm_backup/ && cp -r /scratch/$USER/actdm/datasets/wikipedia/statistics ~/actdm_backup/
```

- Если scratch внезапно почистили посреди экспериментов: заново
  `prefetch_offline.py` + `./run_wikipedia.sh data` + вернуть `*.pth` и
  `statistics/` из `~/actdm_backup` в `datasets/wikipedia/` — обучение
  диффузий продолжится с чекпоинтов в home.
...
Проверяет весь пайплайн до первых метрик, прежде чем жечь GPU-дни.

```bash
NUM_TEXTS=200000 ./run_wikipedia.sh data
./run_wikipedia.sh stats        # дождаться
./run_wikipedia.sh decoders     # дождаться
./run_wikipedia.sh diffusion    # дождаться ПЕРВОГО eval (шаг 12500) и убить
```

В slurm-логе первого eval проверить: SOURCE/TARGET режутся по 64 токена,
примеры генерации не мусор, метрики посчитались, лосс падает.

**Обязательная уборка после смоука** — иначе полный прогон подхватит
смоук-артефакты (датасет, статистики, чекпоинты с теми же именами):

```bash
rm -rf datasets/wikipedia checkpoints/*wikipedia* generated_texts/*wikipedia*
```
...
Это самая долгая стадия (150k итераций диффузии, 50k GPT). Чекпоинты — в
`checkpoints/<prefix>/`, прогресс и промежуточные метрики — в slurm-логах
